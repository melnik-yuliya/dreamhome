 ...
 */        var defaults$1 = {
        ...
        let win$1;
        if ("undefined" !== typeof window) win$1 = window; else if ("undefined" !== typeof __webpack_require__.g) win$1 = __webpack_require__.g; else if ("undefined" !== typeof self) win$1 = self; else win$1 = {};
        var global$2 = win$1;
        function css(el, styles) {
            if ("string" === typeof styles) return global$2.getComputedStyle(el).getPropertyValue(styles);
        ...
        function ready(callback) {
            if ("complete" === document.readyState || "interactive" === document.readyState) callback(); else document.addEventListener("DOMContentLoaded", callback, {
                capture: true,
                once: true,
                passive: true
            });
        ...
        function updateWindowHeight() {
            ...
            if (isMobile()) wndH = getDeviceHeight(); else wndH = global$2.innerHeight || document.documentElement.clientHeight;
        }
        updateWindowHeight();
        ...
        const visibilityObserver = new global$2.IntersectionObserver((entries => {
        ...
        function addObserver(instance) {
            ...
            if (1 === jarallaxList.length) global$2.requestAnimationFrame(updateParallax);
        ...
        const {navigator: jarallax_esm_navigator} = global$2;
        let instanceID = 0;
        class Jarallax {
            constructor(item, userOptions) {
                ...
                Object.keys(dataOptions).forEach((key => {
                    ...
                    if (lowerCaseOption && "undefined" !== typeof self.defaults[lowerCaseOption]) pureDataOptions[lowerCaseOption] = dataOptions[key];
                }));
                self.options = self.extend({}, self.defaults, pureDataOptions, userOptions);
                self.pureOptions = self.extend({}, self.options);
                Object.keys(self.options).forEach((key => {
                    if ("true" === self.options[key]) self.options[key] = true; else if ("false" === self.options[key]) self.options[key] = false;
                }));
            ...
            initImg() {
                ...
                if (!($imgElement instanceof Element)) if (self.options.imgSrc) {
                    $imgElement = new Image;
                    $imgElement.src = self.options.imgSrc;
                } else $imgElement = null;
                if ($imgElement) {
                    if (self.options.keepImg) self.image.$item = $imgElement.cloneNode(true); else {
            ...
            init() {
                ...
                if ("fixed" === this.image.position) self.css(self.image.$container, {
                    "-webkit-clip-path": "polygon(0 0, 100% 0, 100% 100%, 0 100%)",
                    "clip-path": "polygon(0 0, 100% 0, 100% 100%, 0 100%)"
                });
                self.image.$container.setAttribute("id", `jarallax-container-${self.instanceID}`);
                if (self.options.containerClass) self.image.$container.setAttribute("class", self.options.containerClass);
                self.$item.appendChild(self.image.$container);
                if (self.image.useImgTag) imageStyles = self.extend({
                    "object-fit": self.options.imgSize,
                    "object-position": self.options.imgPosition,
                    "max-width": "none"
                }, containerStyles, imageStyles); else {
            ...
            destroy() {
                ...
                if (!originalStylesTag) self.$item.removeAttribute("style"); else self.$item.setAttribute("style", originalStylesTag);
                if (self.image.useImgTag) {
                    const originalStylesImgTag = self.image.$item.getAttribute("data-jarallax-original-styles");
                    self.image.$item.removeAttribute("data-jarallax-original-styles");
                    if (!originalStylesImgTag) self.image.$item.removeAttribute("style"); else self.image.$item.setAttribute("style", originalStylesTag);
            ...
            coverImage() {
                ...
                if (isScroll) resultMT = (wndH - resultH) / 2; else resultMT = (contH - resultH) / 2;
            ...
            onScroll(force) {
                const self = this;
                if (!force && !self.isVisible()) return;
                const {height: wndH} = getWindowSize();
                ...
                if (contH < wndH) visiblePercent = 1 - (afterTop || beforeBottom) / contH; else if (beforeTopEnd <= wndH) visiblePercent = beforeTopEnd / wndH; else if (beforeBottomEnd <= wndH) visiblePercent = beforeBottomEnd / wndH;
                ...
                if ("scale" === self.options.type || "scale-opacity" === self.options.type) {
                    let scale = 1;
                    if (self.options.speed < 0) scale -= self.options.speed * visiblePercent; else scale += self.options.speed * (1 - visiblePercent);
        ...
        const jarallax$1 = function(items, options, ...args) {
            if ("object" === typeof HTMLElement ? items instanceof HTMLElement : items && "object" === typeof items && null !== items && 1 === items.nodeType && "string" === typeof items.nodeName) items = [ items ];
 ...
 */        let win;
        if ("undefined" !== typeof window) win = window; else if ("undefined" !== typeof __webpack_require__.g) win = __webpack_require__.g; else if ("undefined" !== typeof self) win = self; else win = {};
        function Deferred() {
            this.doneCallbacks = [];
            this.failCallbacks = [];
        }
        ...
        function addTouchClass() {
            ...
            for (let i = 0; i < arrow.length; i++) {
                ...
                thisLink.classList.add("parent-arrow");
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            document.querySelector(".button__lang").onclick = function() {
                if ("English - En" == this.textContent) this.textContent = "Russian - Ru"; else this.textContent = "English - En";
            };
            const textInput = document.querySelector("textarea");
            if (textInput) textInput.oninput = function() {
                this.style.height = "auto";
                this.style.height = this.scrollHeight + "px";
            };
        }));
        const rangeInput = document.querySelectorAll(".range-input input"), priceInput = document.querySelectorAll(".price-input input"), range = document.querySelector(".slider-line .progress");
        let priceGap = 1e3;
        priceInput.forEach((input => {
            input.addEventListener("input", (e => {
                let minPrice = parseInt(priceInput[0].value), maxPrice = parseInt(priceInput[1].value);
                if (maxPrice - minPrice >= priceGap && maxPrice <= rangeInput[1].max) if ("input-min" === e.target.className) {
        ...
        rangeInput.forEach((input => {
            input.addEventListener("input", (e => {
                ...
                if (maxVal - minVal < priceGap) if ("range-min" === e.target.className) rangeInput[0].value = maxVal - priceGap; else rangeInput[1].value = minVal + priceGap; else {
        ...
        function ssr_window_esm_extend(target = {}, src = {}) {
            Object.keys(src).forEach((key => {
                if ("undefined" === typeof target[key]) target[key] = src[key]; else if (ssr_window_esm_isObject(src[key]) && ssr_window_esm_isObject(target[key]) && Object.keys(src[key]).length > 0) ssr_window_esm_extend(target[key], src[key]);
            }));
        }
        const ssrDocument = {
            body: {},
        ...
        class Swiper {
            ...
            update() {
                ...
                if (swiper.params.freeMode && swiper.params.freeMode.enabled) {
                    ...
                    if (("auto" === swiper.params.slidesPerView || swiper.params.slidesPerView > 1) && swiper.isEnd && !swiper.params.centeredSlides) {
        ...
        function EffectFlip({swiper, extendParams, on}) {
            ...
            const createSlideShadows = (slideEl, progress, params) => {
                ...
                if (!shadowBefore) shadowBefore = create_shadow_createShadow(params, slideEl, swiper.isHorizontal() ? "left" : "top");
                if (!shadowAfter) shadowAfter = create_shadow_createShadow(params, slideEl, swiper.isHorizontal() ? "right" : "bottom");
        ...
        function initSliders() {
            ...
            new core(".projects-container", {
                ...
                pagination: {
                    el: ".projects-container .swiper-pagination",
                    type: "bullets",
                    clickable: true
                }
            });
            new core(".galary-container", {
                modules: [ Navigation, Pagination ],
                observer: true,
                observeParents: true,
                slidesPerView: "auto",
                ...
                navigation: {
                    prevEl: ".galary-container .swiper-button-prev",
                    nextEl: ".galary-container .swiper-button-next"
                },
                pagination: {
                    el: ".galary-container .swiper-pagination",
                    type: "bullets",
                    clickable: true
                }
            });
